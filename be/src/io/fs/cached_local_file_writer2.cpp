...
#include "cached_local_file_writer2.h"
...
CachedLocalFileWriter2::CachedLocalFileWriter2(Path path, int fd, FileSystemSPtr fs)
...
CachedLocalFileWriter2::CachedLocalFileWriter2(Path path, int fd)
        : CachedLocalFileWriter2(path, fd, global_local_filesystem()) {}

CachedLocalFileWriter2::~CachedLocalFileWriter2() {
    if (_opened) {
        close();
    }
...
Status CachedLocalFileWriter2::close() {
    return _close(true);
}

Status CachedLocalFileWriter2::abort() {
    RETURN_IF_ERROR(_close(false));
    return io::global_local_filesystem()->delete_file(_path);
}

Status CachedLocalFileWriter2::appendv(const Slice* data, size_t data_cnt) {
    DCHECK(!_closed);
    _dirty = true;
...
Status CachedLocalFileWriter2::write_at(size_t offset, const Slice& data) {
    DCHECK(!_closed);
    _dirty = true;
...
Status CachedLocalFileWriter2::finalize() {
    DCHECK(!_closed);
    if (_dirty) {
...
Status CachedLocalFileWriter2::_close(bool sync) {
    if (_closed) {
        return Status::OK();
    }
    _closed = true;
