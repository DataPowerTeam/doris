...
void PInternalService::get_pulsar_info(google::protobuf::RpcController* controller,
                                           ...
                                           google::protobuf::Closure* done) {
    int timeout_ms = request->has_timeout() ? request->timeout() * 1000
                                            : config::routine_load_pulsar_timeout_second * 1000;
    ...
    if (!st.ok()) {
        LOG(WARNING) << "get pulsar info: " << st << " ,timeout: " << timeout_ms
                     << ", thread pool size: "
                     << _exec_env->send_batch_thread_pool()->num_threads();
        brpc::ClosureGuard closure_guard(done);
        Status::Error<ErrorCode::SERVICE_UNAVAILABLE>(
                fmt::format("too busy to get pulsar info, please check the pulsar status, "
                            "timeout ms: {}",
                            timeout_ms))
...
void PInternalService::_get_pulsar_info_impl(const PPulsarProxyRequest* request,
    ...
    if (request->has_pulsar_meta_request()) {
        ...
        Status st = _exec_env->routine_load_task_executor()->get_pulsar_partition_meta(
                request->pulsar_meta_request(), &partitions);
        if (st.ok()) {
